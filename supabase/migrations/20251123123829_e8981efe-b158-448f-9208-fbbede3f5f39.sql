-- Add in-app notification to admin_delete_parking_listing_complete
...
BEGIN
  ...
  INSERT INTO public.user_messages (
    ...
    deleted_record.owner_id,
    'Parking Listing Delisted',
    'Your parking listing "' || deleted_record.title || '" at ' || deleted_record.address || ' (' || deleted_record.zone || ') has been delisted by the admin. If you have any questions, please contact us through the support channel.',
    true,
    false
  );

  -- Send delisting notification email (async, non-blocking)
  BEGIN
  ...
  result := json_build_object(
    ...
    'message', 'Listing deleted successfully, owner notified via email and in-app message'
  );
