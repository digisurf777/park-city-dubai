-- Revert get_parking_listings_with_availability to use 'approved' status (was changed to 'published')
...
BEGIN
  ...
  WHERE pl.status = 'approved'  -- reverted from 'published' back to original behavior
