...
DROP POLICY IF EXISTS "Users can insert their own parking images" ON storage.objects;
DROP POLICY IF EXISTS "Users can insert their own verification documents" ON storage.objects;
DROP POLICY IF EXISTS "Users can update their own verification documents" ON storage.objects;
DROP POLICY IF EXISTS "Users can insert own verification docs" ON storage.objects;
DROP POLICY IF EXISTS "Admins can manage all verification docs" ON storage.objects;

-- Create new policies with proper UUID handling for parking images
CREATE POLICY "Users can view parking images" 
ON storage.objects 
FOR SELECT 
...
-- Create new policies for verification documents  
