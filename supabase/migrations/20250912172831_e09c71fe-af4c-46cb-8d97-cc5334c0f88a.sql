-- Fix the update_parking_space_status function to resolve ambiguous column reference
...
BEGIN
  ...
  -- Update the space with explicit column references
  UPDATE public.parking_spaces 
  SET 
    space_status = new_status,
    override_status = is_override,
    override_reason = CASE WHEN is_override THEN update_parking_space_status.override_reason ELSE NULL END,
  ...
  INSERT INTO public.parking_space_audit_log (
    ...
    auth.uid(),
    is_override,
    update_parking_space_status.override_reason,
    now()
  );
