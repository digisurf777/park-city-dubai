-- Update the admin_delete_parking_listing_complete function to notify customers
...
BEGIN
  ...
  IF owner_record.email IS NOT NULL THEN
    PERFORM net.http_post(
      ...
      body := jsonb_build_object(
        'ownerEmail', owner_record.email,
        'ownerName', owner_record.full_name,
        'listingTitle', deleted_record.title,
        'listingZone', deleted_record.zone
      )
    );
  END IF;

  -- Delete related records
  DELETE FROM public.parking_spaces WHERE parking_spaces.listing_id = admin_delete_parking_listing_complete.listing_id;
  DELETE FROM public.deposit_payments WHERE deposit_payments.listing_id = admin_delete_parking_listing_complete.listing_id;
  DELETE FROM public.driver_owner_messages WHERE driver_owner_messages.listing_id = admin_delete_parking_listing_complete.listing_id;
  ...
  result := json_build_object(
    ...
    'message', 'Listing deleted successfully. Owner and ' || customers_notified || ' customer(s) notified.'
  );
