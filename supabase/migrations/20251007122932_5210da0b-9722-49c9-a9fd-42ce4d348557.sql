-- Drop and recreate get_pre_authorization_overview to include proper phone number fetching
...
BEGIN
  ...
  WHERE pb.status NOT IN ('cancelled', 'rejected')
    AND pb.payment_status IN ('pending', 'payment_sent', 'pre_authorized', 'confirmed', 'partially_captured')
    AND (pb.pre_authorization_amount IS NOT NULL OR pb.payment_amount_cents IS NOT NULL)
