-- Harden and disambiguate unified customer payment history RPC
...
BEGIN
  ...
  all_users AS (
    SELECT DISTINCT driver_stats.user_id FROM driver_stats
    UNION
    SELECT DISTINCT owner_stats.user_id FROM owner_stats
  ),
