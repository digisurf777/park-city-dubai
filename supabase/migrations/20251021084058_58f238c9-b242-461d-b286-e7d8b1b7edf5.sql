-- Update get_unified_customer_payment_history to only show active customers
CREATE OR REPLACE FUNCTION public.get_unified_customer_payment_history()
 RETURNS TABLE(user_id uuid, full_name text, email text, user_type text, driver_bookings_count bigint, owner_payments_count bigint, total_driver_spent numeric, total_owner_received numeric, verification_status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
